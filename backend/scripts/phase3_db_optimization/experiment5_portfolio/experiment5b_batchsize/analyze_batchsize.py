#!/usr/bin/env python3
"""
Experiment 5b (Batch Size) vs Baseline 비교 분석
"""
...
BATCHSIZE_DIR = BASE_DIR / "batchsize_result"
...
def load_batchsize():
    """Batch Size 데이터 로드"""
    p95_df = pd.read_csv(BATCHSIZE_DIR / "p95_response_time.csv")
    avg_df = pd.read_csv(BATCHSIZE_DIR / "avg_response_time.csv")
    status_df = pd.read_csv(BATCHSIZE_DIR / "status_code_distribution.csv")
...
def main():
    print("=" * 70)
    print("Experiment 5b: Batch Size vs Baseline 성능 비교")
    print("=" * 70)
    print()
    ...
    improvement_p95 = ((baseline_p95 - batchsize_p95) / baseline_p95 * 100) if baseline_p95 > 0 else 0

    print(f"Baseline:   {baseline_p95:8.2f} ms")
    print(f"Batch Size: {batchsize_p95:8.2f} ms")
    ...
    improvement_avg = ((baseline_avg - batchsize_avg) / baseline_avg * 100) if baseline_avg > 0 else 0

    print(f"Baseline:   {baseline_avg:8.2f} ms")
    print(f"Batch Size: {batchsize_avg:8.2f} ms")
    ...
    improvement_error = baseline_error - batchsize_error

    print(f"Baseline:   {baseline_error:8.2f}%")
    print(f"Batch Size: {batchsize_error:8.2f}%")
    ...
    success_criteria = {
        "P95 < 100ms": batchsize_p95 < 100,
        "에러율 < 5%": batchsize_error < 5,
        "P95 개선 > 90%": improvement_p95 > 90,
        "에러율 완전 제거": batchsize_error == 0
    }
    ...
    print("\n[P95 응답 시간 분포]")
    if batchsize['p95']:
        print(f"  최소값:  {min(batchsize['p95']):8.2f} ms")
        print(f"  최대값:  {max(batchsize['p95']):8.2f} ms")
        print(f"  중앙값:  {np.median(batchsize['p95']):8.2f} ms")
        print(f"  표준편차: {np.std(batchsize['p95']):8.2f} ms")

    print("\n[평균 응답 시간 분포]")
    if batchsize['avg']:
        print(f"  최소값:  {min(batchsize['avg']):8.2f} ms")
        print(f"  최대값:  {max(batchsize['avg']):8.2f} ms")
        print(f"  중앙값:  {np.median(batchsize['avg']):8.2f} ms")
        print(f"  표준편차: {np.std(batchsize['avg']):8.2f} ms")
