/**
 * Experiment 4: Baseline with Mock API
 *
 * 목적: Mock API 환경에서 Clean Baseline 측정 (500 독립 유저)
 * 설정: Mock API (Rate Limit 없음), 캐시 ON
 * 부하: 0 → 500 VUser (Step-up)
 *
 * 실행:
 *   k6 run --out influxdb=http://192.168.0.5:8086/k6 exp4_baseline_mock.js
 */
...
export function setup() {
  ...
  console.log('Experiment 4: Baseline with Mock API');
  console.log('='.repeat(60));
...
export function teardown(data) {
  console.log('');
  console.log('Experiment 4 Baseline Test Completed');
...
export function handleSummary(data) {
  ...
  console.log('');
  console.log('='.repeat(60));
  console.log('Experiment 4: Baseline with Mock API - Summary');
  console.log('='.repeat(60));
  ...
  return {
    ...
    './exp4_baseline_mock_summary.json': JSON.stringify(data, null, 2),
  };
}
