/**
 * Experiment 4b: Mock API with Connection Pool
 *
 * 목적: Connection Pool 적용 후 성능 측정
 * 변경: SimpleClientHttpRequestFactory → Apache HttpClient Connection Pool
 * 설정: Mock API, 캐시 ON, Connection Pool ON
 * 부하: 0 → 500 VUser (Step-up)
 *
 * 실행:
 *   k6 run --out influxdb=http://192.168.0.5:8086/k6 exp4b_mock_connectionpool.js
 */
...
export const options = {
  ...
  tags: {
    experiment: 'experiment04b',
    phase: 'mock_connectionpool',
    api: 'mock',
  },
};
...
export function setup() {
  ...
  console.log('Experiment 4b: Mock API with Connection Pool');
  console.log('='.repeat(60));
  ...
  console.log('  - Connection Pool: Apache HttpClient (200 max, 50 per route)');
  console.log('='.repeat(60));
...
export function teardown(data) {
  console.log('');
  console.log('Experiment 4b Connection Pool Test Completed');
...
export function handleSummary(data) {
  ...
  console.log('');
  console.log('='.repeat(60));
  console.log('Experiment 4b: Mock API with Connection Pool - Summary');
  console.log('='.repeat(60));
  ...
  return {
    ...
    './exp4b_mock_connectionpool_summary.json': JSON.stringify(data, null, 2),
  };
}
