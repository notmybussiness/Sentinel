/**
 * Experiment 4d: Service Layer Cache (10 seconds)
 *
 * 목적: 짧은 TTL + Service 레벨 캐싱으로 병목 해결
 * 변경:
 *   - Provider @Cacheable 제거 (sync=true 제거)
 *   - Service @Cacheable 추가 (10초 TTL)
 *   - Connection Pool: 500/200
 * 설정: Mock API, Service Layer Cache (10초)
 * 부하: 0 → 500 VUser (Step-up)
 *
 * 가설:
 *   - Service 레벨 캐싱으로 Provider 호출 회피
 *   - sync=true 제거로 락 경합 해소
 *   - 10초 TTL로 Stale data 최소화
 *
 * 실행:
 *   k6 run --out influxdb=http://192.168.0.5:8086/k6 exp4d_service_cache_10s.js
 */
...
export const options = {
  ...
  tags: {
    experiment: 'experiment04d',
    phase: 'service_cache_10s',
    api: 'mock',
  },
};
...
export function setup() {
  ...
  console.log('Experiment 4d: Service Layer Cache (10 seconds)');
  console.log('='.repeat(60));
  ...
  console.log('');
  console.log('Changes from 4c:');
  console.log('  - Cache Layer: Provider → Service');
  console.log('  - TTL: 3 minutes → 10 seconds');
  console.log('  - sync=true: REMOVED');
  console.log('  - Connection Pool: 500/200 (same)');
  console.log('='.repeat(60));
...
export function teardown(data) {
  console.log('');
  console.log('Experiment 4d Service Cache Test Completed');
...
export function handleSummary(data) {
  ...
  console.log('');
  console.log('='.repeat(60));
  console.log('Experiment 4d: Service Cache (10s) - Summary');
  console.log('='.repeat(60));
  ...
  return {
    ...
    './exp4d_service_cache_10s_summary.json': JSON.stringify(data, null, 2),
  };
}
...
function textSummary(data) {
  ...
  return '\n' +
    'Experiment 4d: Service Cache (10s)\n' +
    '==================================\n' +
