 ...
 * - WebSocket: GET /api/v1/crypto/stream/prices?symbols=BTC,ETH&method=WebSocket
 * - LongPolling: GET /api/v1/crypto/stream/prices?symbols=BTC,ETH&method=LongPolling
 */
@RestController
...
public class CryptoStreamController {
    ...
    public Flux<ServerSentEvent<CryptoPriceDto>> streamPrices(
            @Parameter(description = "암호화폐 심볼 (쉼표로 구분)", example = "BTC,ETH,XRP")
            @RequestParam String symbols,

            @Parameter(description = "기준 통화", example = "KRW")
            @RequestParam(defaultValue = "KRW") String baseCurrency,

            @Parameter(description = "스트리밍 방식 (SSE, WebSocket, LongPolling)", example = "SSE")
            @RequestParam(defaultValue = "SSE") String method
    ) {
        ...
        return service.startStreaming(symbolList, baseCurrency)
