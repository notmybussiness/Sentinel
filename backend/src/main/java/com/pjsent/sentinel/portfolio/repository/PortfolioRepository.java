...
public interface PortfolioRepository extends JpaRepository<Portfolio, Long> {
    ...
    /**
     * 사용자 ID로 포트폴리오 목록 조회 (Entity Graph - N+1 해결)
     * 
     * @EntityGraph를 사용하여 Holdings를 함께 로딩
     */
    @EntityGraph(attributePaths = { "holdings" })
    @Query("SELECT p FROM Portfolio p WHERE p.userId = :userId ORDER BY p.createdAt DESC")
    List<Portfolio> findByUserIdOrderByCreatedAtDescWithHoldings(@Param("userId") Long userId);
    ...
    /**
     * 사용자 ID와 포트폴리오 ID로 포트폴리오 조회 (Entity Graph - N+1 해결)
     * 
     * @EntityGraph를 사용하여 Holdings를 함께 로딩
     */
    @EntityGraph(attributePaths = { "holdings" })
    ...
    /**
     * 특정 심볼을 보유한 포트폴리오 조회 (EDA용)
     */
    @EntityGraph(attributePaths = { "holdings" })
    List<Portfolio> findByHoldings_Symbol(String symbol);
}
