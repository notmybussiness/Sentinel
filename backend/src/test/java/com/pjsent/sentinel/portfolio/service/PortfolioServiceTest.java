...
class PortfolioServiceTest {
        ...
        void setUp() {
                ...
                portfolioDto = PortfolioDto.builder()
                                ...
                                .name("테스트 포트폴리오")
                                .description("테스트용 포트폴리오")
                                .holdings(List.of(holdingDto))
                                .build();

                // Default mapper stubs (lenient because not all tests use them)
                lenient().when(portfolioMapper.toDto(any(Portfolio.class))).thenReturn(portfolioDto);
                lenient().when(portfolioMapper.toHoldingDto(any(PortfolioHolding.class))).thenReturn(holdingDto);
        }
        ...
        void should_ReturnPortfolioList_When_GetPortfoliosByUserId() {
                // Given
                when(portfolioRepository.findByUserIdOrderByCreatedAtDescWithHoldings(userId))
        ...
        void should_UpdateHolding_When_ValidRequest() {
                ...
                // When
                PortfolioHoldingDto result = portfolioService.updateHolding(portfolioId, holding.getId(), userId,
                                request);
