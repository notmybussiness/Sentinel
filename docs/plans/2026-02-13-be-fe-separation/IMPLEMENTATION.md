...
## 설계 결정 요약
- Contract-first: API 계약을 먼저 고정하고 구현은 계약을 따라간다.
- Read/Write 분리: 조회는 pure read, 갱신/발행은 explicit write path로 분리한다.
- Error 표준화: 모든 예외 응답을 `ApiErrorResponse`로 통일한다.
- FE 타입 단일화: 수동 타입 대신 generated model을 기준으로 사용한다.
